...
function getAllAlbums() {
    ...
    authReq.Execute(reqPrepare)
      .then(response => {
        ...
        console.log(response);
        for(let i = 0; i < response.length; i++){
          let template = `<a href="#" onClick="getAllPhotos('${response[i].id}')">${response[i].name}</a>`;
          list.insertAdjacentHTML('beforeend',template);
        }
...
const FBLog = {
  login(){
    FB.login(function(response) {
      ...
      scope: 'email,user_likes,user_photos,manage_pages,publish_pages, publish_actions',
      return_scopes: true
    });
    return false;
  },
